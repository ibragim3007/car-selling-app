...
const FilterComponent = ({ filter }: FilterComponentProps) => {
  ...
  const onLayout = (event: LayoutChangeEvent) => {
    ...
    setShowButton(height > HEIGHT_FILTER);
  };
      ...
      >
        <LayoutAnimation>
          <Grid
            ...
            space="sm"
            style={{ maxHeight: isOpen ? 'auto' : HEIGHT_FILTER, overflow: 'hidden' }}
          >
            <Divider />
            <Grid space="lg">
        ...
        {showButton && (
          <Button onPress={() => setIsOpen(!isOpen)} style={{ marginTop: 10 }} variant="text" color="black">
            ...
            <ArrowDown style={{ transform: [{ rotate: isOpen ? '0deg' : '180deg' }] }} />
          </Button>
        )}
      </CardTitle>
    </View>
